...
function _do_artifactory_repo_cmd_help() {
  ...
  case "${sub_cmd}" in
    ...
    install)
      echo "Builds artifactory docker image to local." ;;
    ...
    start)
      echo "Starts the artifactory server.";;

    stop)
      echo "Stops the artifactory server.";;
  esac
}
