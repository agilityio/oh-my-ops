...
function _do_drone_repo_cmd_help() {
  ...
  case "${sub_cmd}" in
    ...
    install)
      echo "Builds drone docker image to local." ;;
    ...
    start)
      echo "Starts the drone server.";;

    stop)
      echo "Stops the drone server.";;
  esac
}
