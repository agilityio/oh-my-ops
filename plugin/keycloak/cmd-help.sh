...
function _do_keycloak_repo_cmd_help() {
  ...
  case "${sub_cmd}" in
    ...
    install)
      echo "Builds keycloakdb docker image to local." ;;
    ...
    start)
      echo "Starts the keycloak server.";;

    stop)
      echo "Stops the keycloak server.";;
  esac
}
