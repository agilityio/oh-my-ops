...
function _do_sftp_repo_cmd_help() {
  ...
  case "${sub_cmd}" in
    ...
    install)
      echo "Builds sftp docker image to local." ;;
    ...
    start)
      echo "Starts the sftp server.";;

    stop)
      echo "Stops the sftp server.";;
  esac
}
