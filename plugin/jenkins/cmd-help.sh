...
function _do_jenkins_repo_cmd_help() {
  ...
  case "${sub_cmd}" in
    ...
    install)
      echo "Builds jenkins docker image to local." ;;
    ...
    start)
      echo "Starts the jenkins server.";;

    stop)
      echo "Stops the jenkins server.";;
  esac
}
