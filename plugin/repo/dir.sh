...
function _do_repo_dir_add() {
    ...
    if [[ -z "${repo}" ]]; then 
        ...
        if [[ idx -lt 0 ]]; then 
            ...
            local parent_repo=$(_do_array_get '_repo_name' "${idx}")
...
function _do_repo_name_list() {
    ...
    if [[ -z "${name}" ]]; then 
        ...
        local idx=$(_do_array_index_of '_repo_name' "${name}")
        [[ idx -ge 0 ]] || _do_assert_fail "Invalid repo name ${name}."


        local dir=$(_do_array_get '_repo_dir' "${idx}")
...
function _do_repo_list() {
    ...
    if [[ -z "${dir}" ]]; then 
        for d in $(_do_array_print '_repo_dir'); do 
            local name=$(_do_array_get '_repo_name' "${i}")
            echo "${d} ${name}"
            let i+=1
        done
    else 
...
function _do_repo_get() {
    ...
    # Founds a sub alias
    local dir=$(_do_array_get '_repo_dir' "${idx}") 
    local name=$(_do_array_get '_repo_name' "${idx}")
    echo "${dir} ${name}"
}
...
function _do_repo_dir_get() {
    ...
    for n in $(_do_array_print '_repo_name'); do 
        if [[ "${name}" == "${n}" ]]; then 
            ...
            local dir=$(_do_array_get '_repo_dir' "${i}") 
            echo "${dir}"
            return
        fi
        let i+=1
